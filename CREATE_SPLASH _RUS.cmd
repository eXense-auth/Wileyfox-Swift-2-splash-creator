...
src\bin\ffmpeg -hide_banner -loglevel -8 -y -i src\2.png	-f rawvideo -vcodec rawvideo -pix_fmt bgr24 -vf "scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,elbg=32"		"src\tmp\pic2.raw" > NUL
src\bin\ffmpeg -hide_banner -loglevel -8 -y -i src\3.png	-f rawvideo -vcodec rawvideo -pix_fmt bgr24 -vf "scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,elbg=32"		"src\tmp\pic3.raw" > NUL
...
echo.
echo.	Конвертирование RAW в SPLASH образ.....
...
src\bin\dd if=src\tmp\pic4.rle	"of=output\splash.img"	bs=128	seek=1	skip=1	count=3 2>NUL
src\bin\dd if=src\tmp\pic4.rle	"of=output\splash.img"	bs=32	seek=9	skip=1	count=1 2>NUL
src\bin\dd if=src\tmp\pic.rle	"of=output\splash.img"	bs=4	seek=79	skip=39	count=1 2>NUL
echo.
echo.fastboot flash splash splash.img > output\fastboot_flash_splash.cmd
	if exist "output\splash.img" (
		echo.
		echo.	Splash.img создан в папке "output"
		echo.
		) else (
			echo.
			echo.
...
echo.
echo.	Создание ZIP для прошивки.....
...
if exist "output\flashable_splash.zip" (
	echo.
	echo.	ZIP архив для прошивки создан в папке "output".
	echo.	Вы можете прошить flashable_splash.zip
	echo.	через кастомный рекавети TWRP
	echo.
	) else (
		echo.
		echo.	ZIP для прошивки не создан. )
		echo.
		echo.
	)
echo.
...
echo.		!!! ВНИМАНИЕ !!!
echo.	
echo.	ИЗОБРАДЕНИЕ НЕ ПОДХОДИТ
echo.	ДЛЯ СОЗДАНИЯ ЭКРАНА ЗАГРУЗКИ
echo.	
echo.// // // // // // // // // // // // // // // // //
echo.	
echo.	Попробуйте изображение с меньшим количеством
echo.	цветов и меньшей разреженностью цветовых пятен.
echo.	
echo.// // // // // // // // // // // // // // // // //
echo.
echo.
